...
export const integrateSearch = async (query: string, tokenType?: string)=> {
    ...
    const [notes, folders, tags] = await Promise.all([
        ...
        noteRepository.searchNotesByKeyword(tempUserId, query, tokenType),

        // 💡 2. 폴더: 태그 필터 모드일 때는 폴더를 검색하지 않습니다! (빈 배열 반환)
        tokenType ? [] : folderRepository.searchFoldersByName(tempUserId, query),

        // 3. 태그: 버튼이 선택되었을 때만 해당 타입의 태그를 검색
        tokenType ? nodeRepository.searchNodesByLabel(tempUserId, query, tokenType) : []
    ]);
